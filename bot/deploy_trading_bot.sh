...
runcmd:
  ...
  # Download and install trading bot (with fallbacks)
  - |
    cd /tmp
    # Try git clone first
    if git clone https://github.com/KingGekko/trading-bot.git trading-bot-git; then
      mv trading-bot-git/* /opt/trading-bot/
      rm -rf trading-bot-git
    else
...
apt-get update
apt-get install -y curl wget git unzip tar build-essential cmake pkg-config \
    python3 python3-pip protobuf-compiler libprotobuf-dev libssl-dev clang llvm-dev

# Download and install with fallbacks
